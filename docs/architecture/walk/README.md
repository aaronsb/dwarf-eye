...
| surface | `WalkMode::surface` probes `z`, `z-1` then `z+1` for something to stand on and `WalkMode::footing` adds the slab or the ramp fraction |
...
- `WalkMode::footing` re-derives the ramp height the mesher draws:
  `FLOOR_HEIGHT` plus the bilinear patch over `ramp.rs:slopes`. The two are
  cross-checked by `ramp.rs:corner_heights_follow_the_fractions_walk_mode_reads`.
